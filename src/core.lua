...
function Balatest.run_tests(mod, category, after)
	...
	if category then
		...
		if not Balatest.tests_by_mod_and_category[mod] then
			sendErrorMessage(
				"That mod appears to not exist. Aborting. (Are you using your prefix instead of your ID?)",
				"Balatest"
			)
			return
		end
	...
	Balatest.internal.tq(Event({
		...
		no_delete = true,
		func = function()
			if Balatest.internal.done_count ~= #todo then
				return false
			end
			...
			sendInfoMessage(
				pass
					...
					.. fail
					.. " failed"
					.. (skip_count == 0 and "." or (", " .. skip_count .. " skipped.")),
				"Balatest"
			)
...
function Balatest.run_test(test, after)
	...
	Balatest.internal.tq(function()
		...
		elseif Balatest.internal.abort and not Balatest.done[test.name] then
			Balatest.done[test.name] = {
				success = false,
				reason = type(Balatest.internal.abort) == "string" and Balatest.internal.abort or "Aborted",
			}
			Balatest.internal.done_count = Balatest.internal.done_count + 1
